function RGBA(rrggbb, alpha) {
    this.setRGBA(rrggbb, alpha)
}

// STATIC
RGBA.interpolate = function (beginRGBA, finalRGBA, changeAmount) {
    return new RGBA(((1 - changeAmount) * beginRGBA.r + changeAmount * finalRGBA.r) << 16 | ((1 - changeAmount) * beginRGBA.g + changeAmount * finalRGBA.g) << 8 | (1 - changeAmount) * beginRGBA.b + changeAmount * finalRGBA.b, (1 - changeAmount) * beginRGBA.alpha + changeAmount * finalRGBA.alpha)
}

RGBA.randomRGB = function () {
    return new RGBA(Math.random() * 0xFFFFFF)
}

RGBA.randomRGBA = function () {
    return new RGBA(Math.random() * 0xFFFFFF, Math.random())
}
...
RGBA.prototype = {
    ...
    clone: function () {
        return new RGBA(this.getRGB(), this.alpha)
    }
}
